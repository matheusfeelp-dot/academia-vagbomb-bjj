export function FloatingWhatsApp() {
  ...
  return (
    <a
      ...
      className="group fixed bottom-5 right-5 z-50 flex items-center gap-3 overflow-hidden rounded-full border border-whatsapp/40 bg-card/95 py-2 pl-2 pr-2 shadow-lg shadow-black/50 backdrop-blur transition-all duration-300 hover:pr-5 hover:border-whatsapp"
    >
      <span className="relative flex h-11 w-11 shrink-0 items-center justify-center">
        <span className="absolute inset-0 animate-ping rounded-full bg-whatsapp/30" />
        <span className="relative flex h-11 w-11 items-center justify-center rounded-full bg-whatsapp">
          <svg
            viewBox="0 0 24 24"
            className="h-6 w-6 fill-whatsapp-foreground"
            aria-hidden="true"
          >
      ...
      <span className="text-stencil max-w-0 overflow-hidden whitespace-nowrap text-sm font-semibold text-foreground opacity-0 transition-all duration-300 group-hover:max-w-[12rem] group-hover:opacity-100">
        Fale com a gente
      </span>
    </a>
  );
